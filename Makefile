...
install:
	mkdir -p $(DESTDIR)/Library/Wallpapers
	mkdir -p $(DESTDIR)/Library/Icons/boredos
	cp wallpapers/* $(DESTDIR)/Library/Wallpapers/ 2>/dev/null || true
	cp icons/boredos/*.png $(DESTDIR)/Library/Icons/boredos/ 2>/dev/null || true
...
bup:
	...
	@echo 'assets = "/Library"' >> build/package/MANIFEST.toml
	mkdir -p build
